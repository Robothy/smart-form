...
/**
 * Checkbox field type configuration
 * Multi-select from predefined options
 */
export const checkboxFieldType: FieldTypeConfig = {
  type: 'checkbox',
  ...
  // Edit mode - configure the checkbox field
    ...
    return (
      ...
      >
        {selectedOptions && selectedOptions.length > 0 ? (
          ...
          <span style={{ color: '#64748b' }}>No value</span>
        )}
      </div>
    )
  },
  ...
  validateValue: (field: FormFieldData, value: unknown) => {
    ...
    // Check if all values match one of the options
    const validOptions = field.options?.map((opt) => opt.value) || []
    const hasInvalidValue = value.some((v) => !validOptions.includes(v))
    if (hasInvalidValue) {
