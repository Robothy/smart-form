...
/**
 * Form Filling Tools Definition
 *
 * These tools are available on the form filling page and allow
 * the AI assistant to help users fill in form fields.
 */

const formFillingTools: ToolGroupDefinition = {
  name: 'form-filling',
  description: 'Tools for filling out forms on the fill page',
  tools: [
    {
      name: 'setFieldValue',
      ...
      handler: async ({ fieldLabel, value }, context) => {
        const form = context.getState('fillForm')
        const values = context.getState('fillValues') || {}
        const setValue = context.getState('setFieldValue')

        if (!form) {
          throw new Error('表单尚未加载，请稍后再试')
        }
        if (!setValue) {
          throw new Error('Field value setter not available')
        }
        ...
        const field = form.fields.find(
          (f: any) => f.label.toLowerCase().trim() === fieldLabel.toLowerCase().trim()
        )

        if (!field) {
          const availableFields = form.fields.map((f: any) => f.label).join(', ')
          throw new Error(
        ...
        if (field.type === 'radio') {
          ...
          const option = field.options.find(
            (o: any) => o.label.toLowerCase().trim() === valueStr.toLowerCase().trim()
          )

          if (!option) {
            const availableOptions = field.options.map((o: any) => o.label).join(', ')
            throw new Error(
        ...
        if (field.type === 'checkbox') {
          ...
          for (const label of selectedLabels) {
            const option = field.options!.find(
              (o: any) => o.label.toLowerCase().trim() === label
            )
            if (option) {
          ...
          setValue(fieldId, selectedValues)
        ...
        if (field.type === 'date') {
          ...
          setValue(fieldId, dateObj as unknown as string)
    ...
    },
    {
      ...
      handler: async ({ fields }, context) => {
        const form = context.getState('fillForm')
        const setMultipleValues = context.getState('setMultipleFieldValues')

        if (!form) {
          throw new Error('表单尚未加载，请稍后再试')
        }
        if (!setMultipleValues) {
          throw new Error('Multiple field values setter not available')
        }
        ...
        for (const fieldData of fields) {
          try {
            ...
            const field = form.fields.find(
              (f: any) => f.label.toLowerCase().trim() === fieldData.label.toLowerCase().trim()
            )
            ...
            if (field.type === 'radio') {
              ...
              for (const label of selectedLabels) {
                const option = field.options?.find((o: any) => o.label.toLowerCase().trim() === label)
    ...
    },
    {
      ...
      handler: async ({ fieldLabel }, context) => {
        const form = context.getState('fillForm')
        const clearValue = context.getState('clearFieldValue')

        if (!form) {
          throw new Error('表单尚未加载')
        }
        if (!clearValue) {
          throw new Error('Field value clearer not available')
        }
    ...
    },
    {
      ...
      handler: async ({ fieldLabel }, context) => {
        ...
        if (field.type === 'radio') {
          explanation += `\n这是单选字段，请从以下选项中选择一个：\n`
          explanation += field.options?.map((o: any, i: number) => `${i + 1}. ${o.label}`).join('\n') || '无选项'
        } else if (field.type === 'checkbox') {
          explanation += `\n这是多选字段，可以选择多个选项：\n`
          explanation += field.options?.map((o: any, i: number) => `${i + 1}. ${o.label}`).join('\n') || '无选项'
        ...
        if (currentValue) {
          ...
          if (Array.isArray(currentValue)) {
            const labels = field.options?.filter((o: any) => currentValue.includes(o.value)).map((o: any) => o.label)
            displayValue = labels?.join(', ') || currentValue.join(', ')
          } else if (field.type === 'radio' && field.options) {
            const option = field.options.find((o: any) => o.value === currentValue)
    ...
    },
    {
      ...
      handler: async (_args, context) => {
        ...
        const optionalFields = form.fields.filter((f: any) => !f.required)
        const filledFields = form.fields.filter((f: any) => {
          const value = values[f.id!]
          return value !== undefined && value !== '' && (Array.isArray(value) ? value.length > 0 : true)
        })
        const unfilledRequiredFields = requiredFields.filter((f: any) => {
          const value = values[f.id!]
          return value === undefined || value === '' || (Array.isArray(value) && value.length === 0)
        })

        const fieldTypes = form.fields.reduce((acc: Record<string, number>, f: any) => {
          acc[f.type] = (acc[f.type] || 0) + 1
          return acc
        }, {} as Record<string, number>)

        const fieldsList = form.fields.map((f: any) => {
          const value = values[f.id!]
          ...
          if (isFilled) {
            if (Array.isArray(value)) {
              const labels = f.options?.filter((o: any) => value.includes(o.value)).map((o: any) => o.label)
              displayValue = labels?.join(', ') || value.join(', ')
...
// Register the form filling tools
toolRegistry.register(formFillingTools)
